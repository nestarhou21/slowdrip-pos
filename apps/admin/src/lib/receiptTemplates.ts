...
function receiptBody(d: ReceiptData, size: '58mm' | '80mm'): string {
  const s = size === '58mm' ? {
    page: '58mm',
    bodyWidth: '210px',
    baseFontSize: '11px',
  ...
  const copyContent = (label: string) => `
  <div class="copy">
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family: 'Courier New', Courier, monospace; font-size: ${s.baseFontSize}; color: #000; background: #fff; }
  .copy { width: ${s.bodyWidth}; margin: 0 auto; padding: ${s.copyPadding}; page-break-after: always; break-after: page; }
  ...
  ${copyContent('MERCHANT COPY')}
  ${copyContent('CUSTOMER COPY')}
</body></html>`;
}

// Returns combined HTML with merchant + customer copy (one print job, two pages)
export function buildReceiptHtmls(d: ReceiptData): string {
  return receiptBody(d, RECEIPT_PAPER);
}
